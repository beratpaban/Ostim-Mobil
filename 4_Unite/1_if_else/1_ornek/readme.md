...
2. `activity_main.xml` dosyasındaki `EditText` ve `Button` ID'lerinin yukarıda belirtilenlerle aynı olduğundan emin olun.
3. Emulator veya gerçek cihaz bağlayın.
4. Run (Çalıştır) butonuna basın.
5. Uygulama açıldığında iki sayı girip "Kontrol Et" butonuna basın; sonuç bir `Toast` ile görünür.

---

## 6) Sıkça karşılaşılan hatalar & çözümleri

### Hata: `NumberFormatException` (uygulama çöker)

* Sebep: `EditText` boş veya sayı dışında karakter içeriyor.
* Çözüm: Buton tıklama işlemine girmeden önce `EditText`lerin boş olup olmadığını kontrol edin:

```java
String s1 = editTextNumber1.getText().toString().trim();
String s2 = editTextNumber2.getText().toString().trim();
if (s1.isEmpty() || s2.isEmpty()) {
    Toast.makeText(this, "Lütfen her iki alana da sayı girin", Toast.LENGTH_SHORT).show();
    return;
}
```

Ayrıca `try-catch` bloğu ile `Integer.parseInt` çevresini sarmalayabilirsiniz:
...
try {
    ...
    Toast.makeText(this, "Lütfen geçerli bir tam sayı girin", Toast.LENGTH_SHORT).show();
}
```

### Hata: `findViewById` null dönüyor

* Sebep: `setContentView` çağrılmadan önce `findViewById` çağrılmış olabilir veya XML'de ID yanlış.
* Çözüm: `setContentView` çağrısının doğru layout'u yüklediğinden ve ID adlarının eşleştiğinden emin olun.

### Hata: UI sistem çubuklarının altında kalıyor

* Sebep: `ViewCompat.setOnApplyWindowInsetsListener` yoksa veya yanlış kök view kullanıldıysa.
* Çözüm: Root view'in ID'sinin `main` olduğundan ve listener'ın doğru view üzerinde uygulandığından emin olun.

---

## Ek: İyileştirme önerileri

* Kullanıcı hatalarını (boş giriş, harf girme vs.) kontrol et.
* Çok büyük sayılar için `long` tipi veya `BigInteger` kullanmayı düşün.
* Sonucu bir `TextView` içine de yazdırarak `Toast` ile birlikte kullan.
* UI/UX: `inputType="number"` özelliğini `EditText`lerde ayarlayarak sadece sayısal klavye açılmasını sağla.

---

Eğer istersen, bu README'yi `activity_main.xml` örneği ile genişleteyim veya hata yönetimi + örnek UI ekran görüntüleri ekleyeyim.
